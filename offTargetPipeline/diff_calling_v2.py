...
def find_read_sq(r, wd, p):
    ...
    rd_st_i, rd_e_i = -1, 0
    ref_st_i, ref_e_i = r.reference_start - 1, r.reference_start
    for cp in ciga:
        if cp[1] == 'M':
            rd_st_i = rd_e_i
            rd_e_i = rd_st_i + cp[0]
            ref_st_i = ref_e_i
            ref_e_i = ref_st_i + cp[0]
        elif cp[1] == 'I':
            rd_st_i = rd_e_i
            rd_e_i = rd_st_i + cp[0]
        elif cp[1] == 'D':
            ...
            raise Exception("Unexpected cigar operator: " + cp[1])
        pos = wd[0] + p + 1
        if pos >= ref_st_i and pos < ref_e_i:
            pos = rd_st_i + r.query_alignment_start + pos - ref_st_i
            return r.query_sequence[pos:pos+1],\
                r.query_qualities[pos:pos+1]
    return "B", [-1]
    raise Exception("NOT FOUND: rd_st_i:" + str(rd_st_i) + " rd_e_i:"  + str(rd_e_i) + " ref_st_i:" + str(ref_st_i) + " ref_e_i:" + str(ref_e_i) + " wd[0]:" + str(wd[0]) + " wd[1]:" + str(wd[1]) + " p:" + str(p) + " cigar:" + r.cigarstring)

def wd_read_sq(r, wd, diffs):
    ciga = parse_cigar(r.cigarstring)
    ...
    for p in range(wd[1] - wd[0]):
        ...
        try:
            ...
            q.append(15)
            print("!!!!!!", sq, r, wd, diffs, file=sys.stderr)
    for di in diffs:
        if q[di] < QScore:
            return False
    return True
...
def has_mut(bam, r, wd):
    ...
    ref_st_i1 = r.reference_start
    wd_seq1 = window_seq(r.get_reference_sequence(), ref_st_i1, wd)
    ...
    if r.mate_is_mapped:
        mr = bam.mate(r)
        ref_st_i2 = mr.reference_start
        wd_seq2 = window_seq(mr.get_reference_sequence(), ref_st_i2, wd)
        if wd_seq2 != "" and wd_seq2 == wd_seq2.upper()\
         or (r.mapping_quality <= MapQScore\
            and mr.mapping_quality <= MapQScore):
            return True, False
        ...
        if len(dual_diffs) == 0:
            return True, False
    if r.mapping_quality <= MapQScore:
        return False, False
    if r.mate_is_mapped:
        isQua1 = wd_read_sq(r, wd, dual_diffs)
        isQua2 = wd_read_sq(bam.mate(r), wd, dual_diffs)
        if not isQua1 or not isQua2:
            return False, False
    else:
        isQua1 = wd_read_sq(r, wd, diffs1)
        if not isQua1:
            return False, False
    #NEED CONTINUE
    return False, True
            ...
            for sample_names in [self.sample_names[:2], self.sample_names[2:]]:
                ...
                for sample_name in sample_names:
                    if sample_name in sdic:
                        ...
                        mut_a.append(vdic["mutated"])
                        wt_a.append(vdic["total"] - vdic["mutated"])
                if len(mut_a) > 1:
                    sum = mut_a[0] + mut_a[1]
                    if sum > 0 and float(mut_a[0] - mut_a[1]) / sum < 0.5:
                        mut_s = int((mut_a[0] + mut_a[1]) / 2)
                        wt_s = int((wt_a[0] + wt_a[1]) / 2)
                    elif mut_a[0] > mut_a[1]:
                        mut_s = mut_a[0]
                        wt_s = wt_a[0]
                    else:
                        mut_s = mut_a[1]
        ...
        for r in bam.fetch():
            if r.is_mapped:
                ...
                if len(sa) == 11 and (sa[1] > 0 or sa[2] > 0):
                    ...
                    isBadRead, itHasMut = has_mut(bam, r, wd)
                    if not isBadRead and itHasMut:
                        td["mutated"] += 1
                if not isBadRead:
                    td["total"] += 1
        queue.put((sample_name, d))
